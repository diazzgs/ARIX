    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-[#86868B] mt-1">Actividad reciente de la plataforma</p>
        </div>
        {notifications.some((n) => !n.is_read) && (
          <button onClick={markAllRead} className="flex items-center gap-2 text-sm text-[#86868B] hover:text-[#1D1D1F] transition-colors">
            <Check size={16} />
            ...
            >
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <p className="text-xs text-[#86868B]">{new Date(notif.created_at).toLocaleString("es-HN")}</p>
                </div>
                <p className="text-sm font-medium text-[#1D1D1F]">{notif.title}</p>
                <p className="text-sm text-[#86868B] mt-0.5">{notif.message}</p>
              </div>
              {!notif.is_read && (
                <button onClick={() => markRead(notif.id)} className="text-[#86868B] hover:text-[#1D1D1F] mt-1">
                  <Check size={16} />
                </button>
              )}
            </div>
