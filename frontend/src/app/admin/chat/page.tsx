...
import { Trash2, MessageCircle, Plus, X, Search, Store } from "lucide-react";
...
export default function AdminChatPage() {
  ...
  // Nueva conversación con un Admin de Tienda
  const [showNewChat, setShowNewChat] = useState(false);
  const [storeAdmins, setStoreAdmins] = useState<Contact[]>([]);
  ...
  const openNewChatModal = async () => {
    ...
    try {
      const res = await api.get("/chats/contacts?role=ROLE_STORE_ADMIN");
  ...
  const filteredAdmins = storeAdmins.filter((c) =>
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-[#86868B] mt-1 text-sm">Conversaciones con administradores de tienda</p>
        </div>
        <motion.button
      ...
      {/* Modal nueva conversación con un Admin de Tienda */}
      <AnimatePresence>
        {showNewChat && (
          <motion.div
            initial={{ opacity: 0 }}
            ...
            >
              ...
              <div className="relative mb-3">
                ...
                <input
                  ...
                  placeholder="Buscar administrador de tienda..."
              ...
              <div className="space-y-2 max-h-60 overflow-y-auto">
                {loadingContacts ? (
                  ...
                  <div className="text-center py-6">
                    <p className="text-sm text-[#86868B]">No se encontraron administradores de tienda</p>
                  </div>
                ) : (
                  filteredAdmins.map((contact) => (
                    <motion.button
                      key={contact.id}
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 bg-[#F5F5F7] rounded-lg flex items-center justify-center">
                          <Store size={16} className="text-[#1D1D1F]" />
                        </div>
                        <div>
